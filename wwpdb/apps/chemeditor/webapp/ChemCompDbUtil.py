...
class ChemCompDbUtil(MyConnectionBase):
    ...
    def __searchDuplicateIdList(self, myq, valueList):
        ...
        #
        try:
            sql = 'select comp_id from pdbx_chem_comp_descriptor where type = "' + valueList[0] + '" and program = "' \
                + valueList[1] + '" and descriptor = "' + valueList[2] + '"'
            #
    ...
    def __removeOBSCCDs(self, myq, inputCcdIdList):
        ...
        #
        try:
            sql = "select id from chem_comp where (pdbx_release_status != 'OBS') and ((pdbx_replaced_by is null) or (pdbx_replaced_by = '')) and id in ('" \
                + "', '".join(inputCcdIdList) + "')"
            #
            rows = myq.selectRows(queryString=sql)
            #
