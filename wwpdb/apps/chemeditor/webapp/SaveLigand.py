...
class SaveLigand(ChemEditorBase):
    ...
    def GetResult(self):
        ...
        if not os.access(updatedFilePath, os.R_OK):
            return ""
        orgIinstanceFilePath = os.path.join(self.__sessionPath, self.__instanceId, self.__instanceId + ".cif_org")
        if not os.access(orgIinstanceFilePath, os.R_OK):
            os.rename(instanceFilePath, orgIinstanceFilePath)
        else:
