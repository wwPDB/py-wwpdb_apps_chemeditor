...
class UpdateLigand(ChemEditorBase):
    ...
    def __runMetalCoordinationTools(self):
        ...
        outputList = [
            os.path.join(self._sessionPath, "servalcat.cif"),
            os.path.join(self._sessionPath, "metalcoord.json"),
        ]
        ...
        if rt == 0:
            ...
            if os.access(os.path.join(self._sessionPath, "servalcat.cif"), os.R_OK) and os.access(
                os.path.join(self._sessionPath, "metalcoord.json"), os.R_OK
            ):
    ...
    def __updateChemCompAtomCoordinationCategories(self, myBlock, annotationData):
        ...
        allDataList = []
        for dataList in existingAnnotData.values():
    ...
    def __updateChemCompPcmCategory(self, myBlock, pcmList):
        ...
        for dataVec in pcmList:
            ...
            for pcmVec in existingPcmList:
                if (
                    (dataVec[0] == pcmVec[1])
                    and (dataVec[1] == pcmVec[7])
                    and (dataVec[2] == pcmVec[2])
                    and (dataVec[3] == pcmVec[8])
                ):
                    found = True
                    break
            if found:
